local T, C, L, G = unpack(select(2, ...)) -- Import: T - functions, constants, variables; C - config; L - locales
...
local noscalemult = AS.Mult * AS.UIScale
local tslu = 0
local numkids = 0
local bubbles = {}

if AS.Eyefinity then
...
chatbubblehook:SetScript("OnUpdate", function(chatbubblehook, elapsed)
	...
	if tslu > .05 then
		...
		for i, frame in next, bubbles do
			...
			if AS.Eyefinity then frame:SetScale(0.00001) end
		end
	end
end)